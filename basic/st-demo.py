...
def get_data():
    ...
    # 对于每个产品，使用其平均每日销售额随机生成全年的每日销售额值。
    data = pd.DataFrame({})
    ...
    data.index = data.index.date # 去除时间戳，因此索引将显示干净的日期。data.index.date
...
@st.fragment
def show_daily_sales(data): # 添加三个容器以固定函数中元素的高度
    time.sleep(1)
